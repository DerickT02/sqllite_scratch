...
const uint8_t COMMON_NODE_HEADER_SIZE = NODE_TYPE_SIZE + NODE_TYPE_OFFSET + PARENT_POINTER_SIZE;
...
uint32_t * leaf_node_key(void* node, uint32_t);
...
//Internal nodes are nodes with children, and leaf nodes do not have any children 
...
Cursor* table_find(Table* table, uint32_t);
...
void create_new_root(Table* table, uint32_t right_child_page_num){
   ...
   /*Lefrt child has data copied from old root*/
   memcpy(left_child, root, PAGE_SIZE);
   set_root_node(left_child, false);
   
   /*Root node is an internal node with one key and two children*/
...
void serialize_row(Row * source, void* destination){
    ...
    //make sure all bytes are properly initialized
...
uint32_t * internal_node_child(void* node, uint32_t child_num){
    ...
    if(child_num > num_keys){
        printf("Tried to access child_num %d > num keys %d", child_num, num_keys);
        exit(EXIT_FAILURE);
    }
